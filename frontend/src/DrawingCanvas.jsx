...
// /*
//   ╔══════════════════════════════════════════════════════════════╗
//   ║  DrawingCanvas  v4  —  Production Watercolor Fill           ║
//   ╠══════════════════════════════════════════════════════════════╣
//   ║                                                              ║
//   ║  WHAT'S FIXED vs v3:                                         ║
//   ║                                                              ║
//   ║  BUG 1 — Fill order was random (all pixels dumped at once)  ║
//   ║    v3: fillFrontRef = [...regions[ri].pixels]               ║
//   ║        → pop() gives random pixel order, no growth feel     ║
//   ║    v4: Real BFS from region centroid during FILL phase,     ║
//   ║        expanding ring-by-ring outward like ink on paper     ║
//   ║                                                              ║
//   ║  BUG 2 — Color boundary ref changed each iteration          ║
//   ║    v3: colorDiff(idat, i, n) — 'i' is scan cursor, changes ║
//   ║    v4: seedColor fixed at region birth, all BFS comparisons ║
//   ║        use seedColor as anchor → consistent region identity  ║
//   ║                                                              ║
//   ║  BUG 3 — STROKE_THR too aggressive (any channel < 100)      ║
//   ║    v3: any(R,G,B) < 100 → kills grey gradients in photos   ║
//   ║    v4: min(R,G,B) < 55 for raw, dilate R=2 covers fringe   ║
//   ║        Keeps light-grey photo regions fillable               ║
//   ║                                                              ║
//   ║  BUG 4 — Flat alpha blend looked digital                     ║
//   ║    v3: region_avg_color * FILL_ALPHA + white * (1-alpha)    ║
//   ║    v4: Per-pixel image color + 3 watercolor layers:         ║
//   ║        Layer 1 (WASH):   light base coat, high alpha        ║
//   ║        Layer 2 (BODY):   main color, medium alpha           ║
//   ║        Layer 3 (DETAIL): deepen edges, low alpha            ║
//   ║        + Position-based noise (paper grain texture)         ║
//   ║        + Wet-edge darkening near stroke boundaries          ║
//   ║                                                              ║
//   ║  BUG 5 — No stroke preservation after fill                  ║
//   ║    v4: strokes re-drawn on top of fill at end               ║
//   ║        → ink lines stay crisp over color                    ║
//   ║                                                              ║
//   ║  NEW — Soft bleed at region edges (paper bleed effect)      ║
//   ║    Boundary pixels painted at 35% alpha → color seems to    ║
//   ║    bleed slightly past the ink line, like real watercolor   ║
//   ║                                                              ║
//   ║  NEW — Progress: Detecting 0-40%, Painting 40-100%          ║
//   ║    Detection chunked in 8ms slices (non-blocking UI)        ║
//   ║                                                              ║
//   ╚══════════════════════════════════════════════════════════════╝
// */
...
// const DILATE_R     = 2     // dilation radius for stroke mask
// const COLOR_THR    = 50    // max color diff from seed to stay in same BFS region
// const MIN_REGION   = 60    // ignore regions smaller than this (noise)

// // Watercolor layers — applied sequentially per pixel
// // Each layer: { alpha, noiseAmp, edgeDarken }
// const WC_LAYERS = [
//   { alpha: 0.30, noise: 6,  edgeDarken: 0.0 },  // wash: light base
//   { alpha: 0.45, noise: 10, edgeDarken: 0.0 },  // body: main color
//   { alpha: 0.20, noise: 4,  edgeDarken: 0.3 },  // detail: deepen edges
// ]
// const BLEED_ALPHA  = 0.18  // soft bleed past stroke edge
// const BG_THR       = 235   // image pixel is background if all channels > this
// const BG_VAR_THR   = 18    // AND variance between channels < this

// const PX_PER_FRAME = 1200  // BFS pixels colored per rAF
// const BUILD_MS     = 8     // max ms per chunk during detection
// const IMG_POLL_MS  = 200   // ms between image-ready polls
// const IMG_POLL_MAX = 25    // 5 second max wait
...
//   const detectRef    = useRef(null)   // { vis, strokeMask, cdat, idat, cw, ch, scanPos, regions[] }

//   // Paint phase state — one region at a time
//   const paintRef     = useRef(null)   // { regions[], regIdx, front[], vis, cdat, idat, snap, cw, ch, layerIdx }
//   // Per-pixel visited for BFS during paint (separate from detection vis)
//   const paintVisRef  = useRef(null)

//   /* ── Stroke snapshot for re-draw after fill ────────────── */
//   const strokeSnapRef = useRef(null)  // ImageData of strokes-only (before fill)
...
//   const [fillStage, setFillStage]= useState('') // 'Detecting…' | 'Painting…' | 'Finishing…'
...
//   /* ── Build source image at canvas resolution ──────────── */
//   const buildImgData = useCallback((dw, dh) => {
//     if (!colorFillImage || !dw || !dh) { imgDataRef.current = null; imgReadyRef.current = false; return }
...
//   /* ────────────────────────────────────────────────────────
//      BUILD STROKE MASK
//      pixel is raw stroke if min(R,G,B) < STROKE_THR
//      Then dilate by DILATE_R to cover anti-alias grey fringe
//   ────────────────────────────────────────────────────────── */
...
//       const d = i * 4
//       const minC = Math.min(cdat[d], cdat[d+1], cdat[d+2])
//       if (minC < STROKE_THR) raw[i] = 1
//     }
//     for (let y = 0; y < ch; y++) {
//       for (let x = 0; x < cw; x++) {
...
//   /* ────────────────────────────────────────────────────────
//      COLOR DIFFERENCE — used in detection BFS
//      Compares idat pixel at pos n against a fixed seedColor
//      This prevents body+face+background merging
//   ────────────────────────────────────────────────────────── */
//   const colorDiffFromSeed = (idat, n, seed) => {
//     const d = n * 4
//     return Math.abs(idat[d]   - seed.r)
//          + Math.abs(idat[d+1] - seed.g)
//          + Math.abs(idat[d+2] - seed.b)
//   }

//   /* ────────────────────────────────────────────────────────
//      IS BACKGROUND — skip coloring background pixels
//   ────────────────────────────────────────────────────────── */
//   const isBackground = (idat, i) => {
//     const d = i * 4
//     const r = idat[d], g = idat[d+1], b = idat[d+2]
//     const avg = (r + g + b) / 3
//     const variance = Math.abs(r - avg) + Math.abs(g - avg) + Math.abs(b - avg)
//     return r > BG_THR && g > BG_THR && b > BG_THR && variance < BG_VAR_THR
//   }

//   /* ────────────────────────────────────────────────────────
//      POSITION-BASED PAPER GRAIN
//      Uses pixel position to produce consistent grain pattern
//      (unlike Math.random() which changes every frame)
//   ────────────────────────────────────────────────────────── */
//   const paperGrain = (pos, cw, amp) => {
//     const x = pos % cw, y = Math.floor(pos / cw)
//     // Stable noise from position hash
//     const h = (x * 1664525 + y * 1013904223) & 0xffffffff
//     return ((h & 0xff) / 255 - 0.5) * amp * 2
//   }

//   /* ────────────────────────────────────────────────────────
//      DETECTION PHASE — chunked BFS across full canvas
//      Finds closed white regions between strokes.
//      Uses seedColor as fixed color anchor per region.
//   ────────────────────────────────────────────────────────── */
...
//       if (performance.now() - t0 > BUILD_MS) break   // yield

//       const i = st.scanPos++
//       if (vis[i] || strokeMask[i]) continue

//       // BFS — collect this white island
//       // Key: seedColor FIXED at region birth → consistent boundary
//       const di0   = i * 4
//       const seed  = { r: idat[di0], g: idat[di0+1], b: idat[di0+2] }
//       const q     = [i]; vis[i] = 1
//       let qi      = 0
//       let sumR=0, sumG=0, sumB=0, colorN=0
...
//         // Accumulate average image color
//         const di = pos * 4
//         const ir = idat[di], ig = idat[di+1], ib = idat[di+2]
//         const isBg = ir > BG_THR && ig > BG_THR && ib > BG_THR
//                   && Math.abs(ir-ig)+Math.abs(ig-ib) < BG_VAR_THR*2
//         if (!isBg) { sumR+=ir; sumG+=ig; sumB+=ib; colorN++; hasNonBg = true }

//         // 4-connected BFS — stop at stroke OR color boundary from seed
//         const L = px > 0    ? pos-1  : -1
//         const R = px < cw-1 ? pos+1  : -1
//         const U = py > 0    ? pos-cw : -1
//         const D = py < ch-1 ? pos+cw : -1
//         for (const n of [L, R, U, D]) {
//           if (n < 0 || vis[n] || strokeMask[n]) continue
//           if (colorDiffFromSeed(idat, n, seed) > COLOR_THR) continue
//           vis[n] = 1; q.push(n)
//         }
//       }

//       if (q.length < MIN_REGION || !hasNonBg) continue

//       const avgR = colorN > 0 ? Math.round(sumR / colorN) : 255
//       const avgG = colorN > 0 ? Math.round(sumG / colorN) : 255
//       const avgB = colorN > 0 ? Math.round(sumB / colorN) : 255

//       // Find centroid as fill seed
//       const cx = Math.round((bx0 + bx1) / 2)
//       const cy = Math.round((by0 + by1) / 2)
//       // Find actual fillable pixel near centroid (centroid may be on stroke)
//       let seedIdx = -1
//       for (let r2 = 0; r2 <= 20 && seedIdx < 0; r2++) {
//         for (let dy2 = -r2; dy2 <= r2 && seedIdx < 0; dy2++) {
//           for (let dx2 = -r2; dx2 <= r2 && seedIdx < 0; dx2++) {
//             if (Math.abs(dx2) !== r2 && Math.abs(dy2) !== r2) continue
//             const nx = cx+dx2, ny = cy+dy2
//             if (nx < 0 || ny < 0 || nx >= cw || ny >= ch) continue
//             const ni = ny*cw+nx
//             if (!strokeMask[ni] && q.indexOf(ni) >= 0) seedIdx = ni
//           }
//         }
//       }
//       if (seedIdx < 0) seedIdx = q[0]
...
//     // Sort: largest first (body before details)
...
//     // Hand off to paint phase
//     // Build a fresh visited array for paint BFS (separate from detection vis)
//     // const cw = cW, ch = cH
//     const paintVis = new Uint8Array(cw * ch)
//     // Mark strokes as visited so paint BFS won't cross them
//     const sm = st.strokeMask
//     for (let i = 0; i < cw * ch; i++) { if (sm[i]) paintVis[i] = 1 }
...
//       cw, ch,
//     }
//     paintVisRef.current = paintVis
//     detectRef.current = null

//     // Seed the first region's BFS
//     seedRegion(0)
...
//   const seedRegion = (regIdx) => {
//     const pt = paintRef.current; if (!pt) return
//     const { regions, strokeMask, cw, ch } = pt
//     if (regIdx >= regions.length) return

//     const reg = regions[regIdx]
//     // Build fresh per-region visit mask
//     const vis = paintVisRef.current

//     // BFS from region's seedIdx, restricted to reg.pixels set
//     const pixelSet = new Set(reg.pixels)
//     const front = []
//     let s = reg.seedIdx
//     if (!vis[s] && pixelSet.has(s)) { vis[s] = 1; front.push(s) }
//     else {
//       // Find first unvisited pixel in region
//       for (const p of reg.pixels) {
//         if (!vis[p]) { vis[p] = 1; front.push(p); break }
//       }
//     }
//     pt.front    = front
//     pt.pixelSet = pixelSet  // restrict BFS to this region only
//     pt.regIdx   = regIdx
//     pt.layerIdx = 0
//   }

//   /* ────────────────────────────────────────────────────────
//      PAINT TICK — watercolor layers, PX_PER_FRAME per rAF
     
//      For each pixel:
//        1. Get image color at that exact pixel position
//        2. Skip if background (near white with low variance)
//        3. Apply watercolor layer (alpha blend + paper grain)
//        4. Check if pixel is near stroke → apply bleed effect
     
//      When region done → wait one frame (brush lift) → next region
//      After all regions → re-draw strokes on top
//   ────────────────────────────────────────────────────────── */
...
//       // Check if current region is exhausted
//       if (pt.front.length === 0) {
//         // Flush to screen
//         ctx.putImageData(snap, 0, 0)

//         const nextRegIdx = pt.regIdx + 1
//         const pct = 42 + Math.min(55, Math.round(nextRegIdx / regions.length * 55))
//         setFillPct(pct)

//         if (nextRegIdx >= regions.length) {
//           // All regions painted — final step: redraw strokes on top
...
//         // Pause one frame between regions (brush-lift feel)
//         seedRegion(nextRegIdx)
//         fillRafRef.current = requestAnimationFrame(paintTick)
//         return
//       }

//       const pos = pt.front.shift()   // use shift() for natural expansion order
//       const di  = pos * 4
//       const reg = regions[pt.regIdx]

//       // Get per-pixel image color
//       const ir = idat[di], ig = idat[di+1], ib = idat[di+2]
...
//       if (!isBg) {
//         // Is this pixel near a stroke boundary?
//         const px = pos % cw, py = Math.floor(pos / cw)
//         const nearStroke =
...
//         if (nearStroke) {
//           // Bleed effect: soft color just past the ink line
//           const a   = BLEED_ALPHA
//           const gr  = paperGrain(pos, cw, 3)
//           cdat[di]   = Math.round(Math.min(255, Math.max(0, ir*a + cdat[di]*(1-a) + gr)))
//           cdat[di+1] = Math.round(Math.min(255, Math.max(0, ig*a + cdat[di+1]*(1-a) + gr)))
//           cdat[di+2] = Math.round(Math.min(255, Math.max(0, ib*a + cdat[di+2]*(1-a) + gr)))
//           cdat[di+3] = 255
//         } else {
//           // Watercolor layers applied cumulatively
//           // Each layer blends over the previous result
//           for (const layer of WC_LAYERS) {
//             const gr = paperGrain(pos, cw, layer.noise)
//             const a  = layer.alpha
//             // Edge darken: pixels near strokes get slightly darker
//             const edgeFactor = nearStroke ? (1 - layer.edgeDarken) : 1
//             const cr = cdat[di], cg = cdat[di+1], cb = cdat[di+2]
//             cdat[di]   = Math.round(Math.min(255, Math.max(0, ir*a*edgeFactor + cr*(1-a) + gr)))
//             cdat[di+1] = Math.round(Math.min(255, Math.max(0, ig*a*edgeFactor + cg*(1-a) + gr)))
//             cdat[di+2] = Math.round(Math.min(255, Math.max(0, ib*a*edgeFactor + cb*(1-a) + gr)))
//             cdat[di+3] = 255
//           }
//         }
//       }
...
//       // Expand BFS to 4-connected non-stroke, same-region neighbors
//       const px = pos % cw, py = Math.floor(pos / cw)
//       const vis = paintVisRef.current
//       const L = px>0    ? pos-1  : -1
//       const R = px<cw-1 ? pos+1  : -1
...
//       for (const n of [L, R, U, D]) {
//         if (n < 0 || vis[n] || strokeMask[n]) continue
//         if (!pt.pixelSet.has(n)) continue   // stay within this region
//         vis[n] = 1
//         pt.front.push(n)
// //         const dirWeight = Math.cos(reg.axisAngle) * dx + Math.sin(reg.axisAngle) * dy

// // if (Math.random() < 0.7 + dirWeight * 0.3) {
// //   pt.front.push(n)
// // }
//       }
//     }

//     // Flush partial progress to screen so user sees fill growing
//     ctx.putImageData(snap, 0, 0)
//     fillRafRef.current = requestAnimationFrame(paintTick)
//   // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [getCtx])
...
//   /* ────────────────────────────────────────────────────────
//      FINALIZE — redraw strokes on top of color fill
//      This restores the crisp ink lines over the watercolor
//   ────────────────────────────────────────────────────────── */
//   const finalizeStrokes = useCallback(() => {
//     const ctx = getCtx(); if (!ctx) return

//     // If we have the original stroke snapshot, composite it
//     if (strokeSnapRef.current) {
//       const snap = strokeSnapRef.current
//       const sw   = snap.width, sh = snap.height
//       const sdat = snap.data
//       const cdat = paintRef.current?.cdat

//       if (cdat && cdat.length === sdat.length) {
//         // Composite strokes: where stroke pixel is dark, use stroke color
//         for (let i = 0; i < sw * sh; i++) {
//           const d = i * 4
//           const minC = Math.min(sdat[d], sdat[d+1], sdat[d+2])
//           if (minC < 80) {  // stroke pixel
//             const strokeAlpha = 1 - (minC / 80)  // darker = more opaque
//             const a = strokeAlpha * 0.92
...
//   /* ── Fill animation entry point ─────────────────────────── */
//   const startFillAnimation = useCallback(() => {
//     const c = canvasRef.current; if (!c) return

//     if (colorFillImage && !imgReadyRef.current) {
//       if (fillPollRef.current >= IMG_POLL_MAX) {
//         console.warn('[Fill] image not ready — using stroke outlines only')
//         // Proceed without image — will use stroke region average as color
//       } else {
//         fillPollRef.current++
...
//     paintVisRef.current = null

//     const ctx = getCtx()
//     const cw  = c.width, ch = c.height

//     // Snapshot canvas (strokes drawn, rest white)
//     const snap = ctx.getImageData(0, 0, cw, ch)
//     const cdat = snap.data

//     // Save stroke snapshot before we mutate cdat
//     strokeSnapRef.current = ctx.getImageData(0, 0, cw, ch)

//     const idat = imgReadyRef.current && imgDataRef.current
//       ? imgDataRef.current.data
//       : new Uint8ClampedArray(cw * ch * 4).fill(255)  // fallback: white
...
//     detectRef.current = {
//       vis, strokeMask, cdat, idat, cw, ch,
//       snap,   // shared — paintTick will mutate cdat in place
//       scanPos: 0, regions: [],
//     }
...
//     ctx.lineWidth   = pen.lw * (typeof strokeWidthMultiplier === 'number'
...
//       if (pi < pts.length) {
//         if (pi < pts.length - 1) {
//           const mx = (pts[pi][0] + pts[pi+1][0]) / 2
//           const my = (pts[pi][1] + pts[pi+1][1]) / 2
...
//     paintRef.current = null; paintVisRef.current = null
//     strokeSnapRef.current = null; fillPollRef.current = 0
//   }, [])
...
//       if (rafRef.current) cancelAnimationFrame(rafRef.current)
//       resetFill()
//       rafRef.current = null; runningRef.current = false; ctxRef.current = null
//       imgReadyRef.current = false
...
//                   <span style={{ color:'#22d3ee', fontWeight:700 }}>{fillStage} </span>
//                   {fillPct}%
//                 </span>
...
//                                 background:'linear-gradient(90deg,#f5c542,#f97316)',
//                                 transition:'width .08s' }} />
//                 </div>
//                 <span style={{ fontWeight:800, minWidth:36, textAlign:'right',
//                                fontSize:11, color:'#f5c542' }}>{prog}%</span>
//               </>
//             )}
//           </div>
//         )}
...
//             color: fillDone ? '#a78bfa' : '#34d399', fontSize:10,
...
//                            background: fillDone ? '#a78bfa' : '#34d399',
...
//                                letterSpacing:'.08em', textTransform:'uppercase' }}>Speed</span>
...
//                                color:'#f5c542', minWidth:52, background:'rgba(245,197,66,.1)',
//                                border:'1px solid rgba(245,197,66,.2)', borderRadius:6,
//                                padding:'2px 7px', textAlign:'center' }}>{SPEED_LABELS[speed]}</span>
//               </div>
...
//           )}
//           {isDone && !isFilling && (
//             <CB onClick={download} c="green">⬇ Save PNG</CB>
//           )}
//         </div>
//       )}
//     </div>
...
//       padding:'5px 14px', fontSize:10, cursor:disabled ? 'not-allowed' : 'pointer',
//       transition:'all .15s', fontFamily:'inherit', fontWeight:500,
