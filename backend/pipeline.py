...
"""
pipeline.py - v21 FINAL – Full compatibility with main.py + text_to_sketch.py
Includes colouring_book_edges + MediaPipe Face Boost
"""
...
def auto_detect_style(bgr: np.ndarray) -> dict:
    ...
    if lap_var > 250:
        return {"label": "Portrait / Face", "detail_level": 5, "max_strokes": 2200}
    return {"label": "General", "detail_level": 4, "max_strokes": 1600}


# colouring_book_edges - REQUIRED by text_to_sketch.py
def colouring_book_edges(bgr: np.ndarray) -> np.ndarray:
    """Thick clean outlines for colouring book mode."""
    gray = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY)
...
def _trace_paths(skel: np.ndarray, min_len: int = 5) -> list:
...
def _detect_portrait_edges(bgr: np.ndarray, detail_level: int = 5) -> np.ndarray:
    ...
    canny = cv2.Canny(sketch, 8, 30)

    lab = cv2.cvtColor(bgr, cv2.COLOR_BGR2LAB)
    color_edge = cv2.bitwise_or(cv2.Canny(lab[:,:,1], 20, 60), cv2.Canny(lab[:,:,2], 20, 60))

    fused = cv2.bitwise_or(canny, color_edge)
        ...
        try:
            with mp_face_detection.FaceDetection(min_detection_confidence=0.5) as face_detection:
                ...
                if results.detections:
                    ...
                    face_boost = cv2.bitwise_and(fused, fused, mask=face_mask)
                    face_boost = cv2.Canny(cv2.GaussianBlur(gray, (5,5), 0), 5, 25)
                    fused = cv2.bitwise_or(fused, face_boost)
        except Exception as e:
            print(f"[MediaPipe] Face boost failed: {e}")

    k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
    fused = cv2.morphologyEx(fused, cv2.MORPH_CLOSE, k, iterations=4)
    ...
    if np.count_nonzero(skel) < 800:
        skel = cv2.Canny(sketch, 5, 25)
        if SKIMAGE_AVAILABLE:
    ...
    try:
        ...
        if use_hed:
            ...
            if hed is not None:
                ...
                cl = cv2.morphologyEx(dil, cv2.MORPH_CLOSE, k3, iterations=1)
        ...
        paths = _trace_paths(skel, min_len=5)
        paths = _sort_paths(paths, ph)
        if crosshatch:
            paths += crosshatch_shading(bgr_proc, paths, pw, ph)
        ...
        print(f"[Pipeline v21] Done: {len(scaled)} paths | engine={engine} | quality={quality['score']}")
